sap.ui.define([
	...
	return Controller.extend("com.vasudha.controller.uomCreate", {
		 ...
		 */
			onInit: function() {
				...
				this.getRouter().getRoute("uomCreate").attachPatternMatched(this._onRouteMatched, this);
			},
		...
		//	}
			...
			_oData: [{
				...
				addRegion: function(oEvent) {
					...
					if (!(oView.getModel("newUom"))) {
						var oNewProductModel = new sap.ui.model.json.JSONModel();
						oNewProductModel.setData(oNewUom);
						oView.setModel(oNewProductModel, "newUom");
					} else {
						oView.getModel("newUom").setProperty("/uomid", "");
						oView.getModel("newUom").setProperty("/uom", "");
					}
					if (!that.escapePreventDialog) {
						oView.getModel("newUom").refresh();
						var oFormFragment = sap.ui.xmlfragment("com.vasudha.fragments.newUom");
						...
						that.escapePreventDialog = new sap.m.Dialog({
							...
							buttons: [
								new sap.m.Button({
									...
									type: "Accept",
									press: function() {
										var oData = oView.getModel("newUom").getData();
										...
										$.ajax({
											...
											success: function(data) {
												that.getView().setModel(Model.createUomModel(),"uom");
												that.getView().getModel("uom").refresh();
